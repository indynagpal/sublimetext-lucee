...
def has_completions(project_file_name, bean_name):
	project_name = get_project_name(project_file_name)
	if project_name in projects:
		return bean_name.lower() in projects[project_name]["completions"]
	return False

def get_completions(project_file_name, bean_name):
	return projects[get_project_name(project_file_name)]["completions"][bean_name.lower()]
	...
	if project_file_name:
		...
		for symbol in info["dot_context"]:
			if not symbol.is_function:
				if has_completions(project_file_name, symbol.name):
					return CompletionList(get_completions(project_file_name, symbol.name), 'normal')
				break
